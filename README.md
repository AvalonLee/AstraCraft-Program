...
[![entries](https://img.shields.io/badge/entries-40-blue.svg)](site/index.html)
...
不想翻库、不想手敲？把下面这句直接发给你的 AI Agent，它会只安装「天工精选」这**一份**技能（仓库根目录的 `SKILL.md` 单文件），不会把库里 40 个条目注册成技能：
...
> 技能本体即仓库根目录 [SKILL.md](SKILL.md) 这一份文件，Agent 只抓取它即可安装（**不要克隆整库进 skills 目录**，`entries/` 下 40 份是推荐用的「数据」，不是可安装技能）。在线预览站首页顶部也有同款「一键复制」安装条。
