...
### 新增
- **DooTask 开源任务管理与团队协作系统（2026-09-19）**：收录 `business-office/dootask`（AGPL-3.0）。★5.6k 的 PHP（Laravel）+ Vue 自托管任务管理系统：任务、看板、团队 IM、文件与流程一体，网页 / 桌面（electron）/ 移动多端同源，Docker Compose 部署并以 `./cmd` 统一运维（install / update / backup / https / port / mysql backup 等）。收录理由含 Agent 视角：默认分支 `pro` 自带 `AGENTS.md`、`CLAUDE.md` 与 `.claude/skills/` 下 5 个中文运维技能（install / update / backup / release / fix-permission），其中 install 技能采用「前置检查 → 单次确认 → 失败即停」的刚性流程。注意 AGPL-3.0 对外提供服务触发源码开放义务、默认分支为 pro、含 drawio 与 mobile 两个子模块。评级：常规。目录总数由 93 增至 **94**。
